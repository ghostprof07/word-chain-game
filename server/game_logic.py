...
# hızlı olur ve yalnızca oynanan dil belleğe alınır (özellikle mobilde önemli).
SOZLUKLER = {}


def sozluk_getir(dil):
    """Verilen dilin sözlük kümesini döndürür (lazy yükler ve cache'ler)."""
    if dil not in SOZLUKLER:
        SOZLUKLER[dil] = _tek_sozluk_yukle(dil)
    s = SOZLUKLER[dil]
    if not s and dil != 'en':   # o dil yoksa İngilizce'ye düş
        return sozluk_getir('en')
    return s
...
class GameRoom:
    ...
    def __init__(self, oda_kodu, toplam_sure=300, hamle_sure=20, dict_lang='en'):
        ...
        self.dict_lang = dict_lang if dict_lang in ALFABELER else 'en'
        self.sozluk = sozluk_getir(self.dict_lang)
        self.alfabe = ALFABELER.get(self.dict_lang, ALFABELER['en'])
        # Oyun başı için oynanabilir başlangıç harfleri (sözlükten; yoksa tüm alfabe)
        self.bas_harfler = sorted({w[0] for w in self.sozluk if w}) or list(self.alfabe)
